 ...
 * (closed after 120 s of silence), and that state never leaves the job -- Phase 3
 * deliberately emits no {@code CLOSED} alert. What arrives on the wire is always
 ...
 * Phase 4 only implements NEW to ACKNOWLEDGED, so adding the rest later needs no
 * migration.
 */
